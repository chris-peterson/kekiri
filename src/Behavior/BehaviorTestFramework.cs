...
/// <summary>
/// Discovery and execution, spoken to Microsoft.Testing.Platform directly rather than through a
/// host test framework's extensibility points. Owning the TestNode is what lets a scenario's
/// Gherkin shape reach the runner, the IDE, and the reports as data rather than as console text.
/// </summary>
sealed class BehaviorTestFramework : ITestFramework, IDataProducer, IOutputDeviceDataProducer
{
    readonly Assembly _testAssembly;
    readonly GherkinFormatter _formatter;

    public BehaviorTestFramework(Assembly testAssembly, IOutputDevice outputDevice)
    {
        _testAssembly = testAssembly;
        _formatter = new GherkinFormatter(outputDevice, this);
    }

    public string Uid => nameof(BehaviorTestFramework);
    ...
    {
        BeforeTestRun.InvokeAll(_testAssembly);
    ...
    {
        foreach (var scenario in ScenarioFilter.Apply(request.Filter, Scenario.DiscoverIn(_testAssembly)))
        {
    ...
    async Task RunAsync(ExecuteRequestContext context, RunTestExecutionRequest request)
    {
        foreach (var scenario in ScenarioFilter.Apply(request.Filter, Scenario.DiscoverIn(_testAssembly)))
        {
            ...
            var state = outcome.Failure is null
                ...
                : new FailedTestNodeStateProperty(FailureReport.Of(outcome.Failure));
    ...
    /// <summary>
    /// The properties every node carries. Feature grouping is metadata rather than a parent node:
    ...
    static List<IProperty> Describe(Scenario scenario, IProperty state)
    {
        ...
        properties.AddRange(scenario.Tags);
